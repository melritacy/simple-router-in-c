...
struct sr_instance* global_sr_ptr = NULL;

void handle_shutdown(int sig) {
    printf("\nCaught SIGINT, cleaning up...\n");
    if (global_sr_ptr) {
        sr_destroy_instance(global_sr_ptr);  
    }
    exit(0);  
}
...
int main(int argc, char **argv)
{
    ...
    while (1) {
        ...
        FD_ZERO(&read_fds);
        FD_SET(sr.sockfd, &read_fds);

        timeout.tv_sec = 1;
        timeout.tv_usec = 0;

        int ready = select(sr.sockfd + 1, &read_fds, NULL, NULL, &timeout);

        if (ready > 0 && FD_ISSET(sr.sockfd, &read_fds)) {
            int result = sr_read_from_server(&sr);
            if (result != 1) {
                break;
            }
            ...
            if (difftime(now, last_activity) >= IDLE_TIMEOUT_SEC) {
                fprintf(stderr, "Idle for %d seconds, shutting down.\n", IDLE_TIMEOUT_SEC);
                break;
            } 
        }
    }
